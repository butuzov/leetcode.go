...
func TestTallyHappy(t *testing.T) {
	for _, tt := range happyTestCases {
		...
		if err != nil {
			t.Errorf("Tally for input named %q returned error %q. Error not expected.",
				tt.description, err)
		}
		if actual != tt.expected {
			t.Errorf("Tally for input named %q was expected to return...\n%s\n...but returned...\n%s",
...
func TestTallyError(t *testing.T) {
	for _, s := range errorTestCases {
		...
		err := Tally(reader, &buffer)
		if err == nil {
			t.Errorf("Tally for input %q should have failed but didn't.", s)
		}
		var _ error = err
	}
}
