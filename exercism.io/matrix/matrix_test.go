...
func TestNew(t *testing.T) {
	for _, test := range tests {
		...
		case err != nil:
			var _ error = err
			if test.ok {
				t.Errorf("New(%q) returned error %q.  Error not expected",
					test.in, err)
			}
		case !test.ok:
			t.Errorf("New(%q) = %v, %v.  Expected non-nil error.",
				test.in, m, err)
		case m == nil:
			t.Errorf("New(%q) = %v, want non-nil *Matrix",
				test.in, m)
		}
	}
}
...
func TestRows(t *testing.T) {
	for _, test := range tests {
		...
		if !reflect.DeepEqual(r, test.rows) {
			t.Errorf("New(%q).Rows() = %v, want %v", test.in, r, test.rows)
		}
		if len(r[0]) == 0 {
		...
		if !reflect.DeepEqual(m.Rows(), test.rows) {
			t.Errorf("Matrix.Rows() returned slice based on Matrix " +
...
func TestCols(t *testing.T) {
	for _, test := range tests {
		...
		if !reflect.DeepEqual(c, test.cols) {
			t.Errorf("New(%q).Cols() = %v, want %v", test.in, c, test.cols)
		}
		if len(c[0]) == 0 {
			continue // not currently in test data, but anyway
		}
		...
		if !reflect.DeepEqual(m.Cols(), test.cols) {
			t.Errorf("Matrix.Cols() returned slice based on Matrix " +
				"representation.  Want independent copy of element data.")
		}
	}
}
	...
	for r := 0; r < 3; r++ {
		for c := 0; c < 3; c++ {
			...
			if ok := m.Set(r, c, val); !ok {
				t.Errorf("Matrix(%q).Set(%d, %d, %d) returned !ok, want ok.",
					s, r, c, val)
			}
			xr = [][]int{{1, 2, 3}, {4, 5, 6}, {7, 8, 9}}
			xc = [][]int{{1, 4, 7}, {2, 5, 8}, {3, 6, 9}}
			...
			if res := m.Rows(); !reflect.DeepEqual(res, xr) {
				t.Errorf("Matrix(%q).Set(%d, %d, %d), Rows() = %v, want %v",
					s, r, c, val, res, xr)
			}
			if res := m.Cols(); !reflect.DeepEqual(res, xc) {
				t.Errorf("Matrix(%q).Set(%d, %d, %d), Cols() = %v, want %v",
					s, r, c, val, res, xc)
			}
		}
	}
	...
	for _, r := range []int{-2, -1, 0, 3, 4} {
		for _, c := range []int{-2, -1, 0, 3, 4} {
			...
			if ok := m.Set(r, c, 0); ok {
				t.Errorf("Matrix(%q).Set(%d, %d, 0) = ok, want !ok", s, r, c)
			}
		}
	}
}
